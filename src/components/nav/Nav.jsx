  ...
  return (
    <nav className={`absolute flex items-center bg-gray-800/40 py-2 left-0 w-full z-10 justify-between px-10 text-white`}>
        <NavLink to="/" className='text-3xl text-black w-32 h-24 p-1 font-medium md:text-alt'>
          <img src={"/logo1.png"} alt="logo" className=' object-cover w-full h-full bg-red-20' /> </NavLink>
        <ul className={`${showMenu && "show"} text-white font-medium text-blac md:text-gray-700  basis-1/2`}>
          ...
          {/* <li  className='px-4 py-2 cursor-pointer font-semibold'><NavLink to={'/services'} className={({isActive}) => (isActive ? "" : "text-white")}> Service </NavLink>
          </li> 
            <li  className='px-4 py-2 cursor-pointer font-semibold'><NavLink to="/about-us" className={({isActive}) => (isActive ? "" : "text-white")}> About Us </NavLink></li>
